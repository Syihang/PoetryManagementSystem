...
public class ModifyPoetryDialog extends JDialog {
    public ModifyPoetryDialog(Frame owner, int id) {
    ...
    private void initDialog(int id) {
        ...
        JLabel southLabel = new JLabel("修改诗词");
    ...
    private void updatePoetry(int id){
        ...
        DataUpdate.PoetryUpdate(id, title, author, dynasty, type, text);
    ...
    private void setAllField(int id){

        Poetry poetry = DataSelece.IDtoPoetry(id);

        assert poetry != null;
        titleField.setText(poetry.getTitle());
        authorField.setText(poetry.getAuthor());
        dynastyComboBox.setSelectedItem(poetry.getDynasty());
        typeComboBox.setSelectedItem(poetry.getType());
        textArea.setText(StrstoString(poetry.getText()));
    }
